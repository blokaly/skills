...
async function saveSnapshot(feedId, items, dataDir = ".crypto-rss-feed/data") {
...
<head>
    ...
    <title>Crypto RSS Feed Report - ${dateStr}</title>
    <style>
        body {
...
<body>
    ...
    <footer>
        Generated by Crypto RSS Feed Agent
    </footer>
</body>
</html>
  `.trim();
...
// src/reporter.ts
async function generateReport(periodHours = 24, dataDir = ".crypto-rss-feed/data", reportDir = ".crypto-rss-feed/report", feedIds) {
...
async function main() {
  ...
  try {
    ...
    const storageRoot = config.defaults.storagePath || import_path3.default.join(import_os.default.tmpdir(), "crypto-rss-feed");
    ...
    console.log(`Crypto RSS Feed: Scanning ${feeds.length} feeds...`);
