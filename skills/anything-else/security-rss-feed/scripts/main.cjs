...
var require_saxes = __commonJS({
  "node_modules/saxes/saxes.js"(exports2) {
    ...
    var SaxesParser2 = class {
       ...
       */
      on(name, handler4) {
        this[EVENT_NAME_TO_HANDLER_NAME[name]] = handler4;
      }
      /**
      ...
      fail(message) {
        ...
        const handler4 = this.errorHandler;
        if (handler4 === void 0) {
          throw err;
        } else {
          handler4(err);
...
`;
      ...
      handleTextInRoot() {
        ...
        const { chunk, textHandler: handler4 } = this;
        scanLoop:
          while (true) {
            switch (this.getCode()) {
              case LESS: {
...
`;
       ...
       */
        ...
        const { text: text2 } = this;
        if (text2.length !== 0) {
          (_a3 = this.textHandler) === null || _a3 === void 0 ? void 0 : _a3.call(this, text2);
          this.text = "";
        }
        this._closed = true;
      ...
      processAttribsNS() {
        ...
        const { attributes: attributes2 } = tag;
        ...
        for (const attr of attribList) {
          ...
          seen.add(eqname);
          attr.uri = uri;
          attributes2[name] = attr;
        }
        this.attribList = [];
      }
      processAttribsPlain() {
        const { attribList } = this;
        const attributes2 = this.tag.attributes;
        for (const { name, value } of attribList) {
          if (attributes2[name] !== void 0) {
      ...
      closeTag() {
        ...
        const handler4 = this.closeTagHandler;
        let l = tags.length;
        while (l-- > 0) {
          const tag = this.tag = tags.pop();
          this.topNS = tag.ns;
          handler4 === null || handler4 === void 0 ? void 0 : handler4(tag);
          if (tag.name === name) {
            break;
          }
...
async function fetchFeed(url, name, cutoffDate) {
  ...
  if (isJson) {
    ...
    try {
      ...
      return cutoffDate ? normalized.filter(
        (item) => new Date(item.pubDate) >= cutoffDate
      ) : normalized;
    } catch (e) {
      throw new Error(
  ...
  return new Promise((resolve, reject) => {
    ...
    parser.on("opentag", (node) => {
      ...
      const tagName19 = node.name.toLowerCase();
      if (tagName19 === "item" || tagName19 === "entry") {
        currentItem = {};
      } else if (currentItem) {
        currentTag = tagName19;
        if (tagName19 === "link" && node.attributes) {
          const hrefAttr = node.attributes.href;
          if (hrefAttr) {
            currentItem.link = hrefAttr;
          }
        }
      }
    });
    parser.on("text", (text2) => {
      if (stopParsing || !currentItem || !currentTag) return;
      if (currentTag === "link" && currentItem.link) return;
      currentItem[currentTag] = (currentItem[currentTag] || "") + text2;
    });
    ...
    parser.on("closetag", (node) => {
      ...
      if (tagName19 === "item" || tagName19 === "entry") {
        if (currentItem) {
          const normalized = normalizeItem(currentItem, name);
          const itemDate = new Date(normalized.pubDate);
    ...
    parser.on("error", (e) => {
      if (!stopParsing) reject(e);
    });
    parser.on("end", () => {
      if (!stopParsing) resolve(items);
    });
    (async () => {
      try {
        if (!response.body) {
...
function normalizeItem(item, source) {
  const getField = (keys2) => {
    for (const key2 of keys2) {
      if (item[key2] !== void 0) return item[key2];
      if (item[key2.toLowerCase()] !== void 0) return item[key2.toLowerCase()];
    }
    return void 0;
  };
  const extractText = (val) => {
    ...
    if (typeof val === "string") {
      ...
      text2 = String(val["#text"]);
    } else {
      text2 = val ? String(val) : "";
    }
  ...
  const title = extractText(getField(["title", "headline"]) || "");
  const linkVal = getField(["link"]);
  const link = typeof linkVal === "string" ? linkVal : "";
  const pubDateRaw = getField(["pubDate", "published", "updated"]) || Date.now();
  let pubDate;
  ...
  return {
    ...
    description: truncateDescription(
      extractText(getField(["description", "summary"]) || "")
    )
  };
}
...
async function main() {
  ...
  if (majorVersion < 24) {
    console.error(
      `Error: This application requires Node.js v24 or higher (detected ${process.version}).`
    );
    console.error(
      "Please install the necessary Node.js runtime from https://nodejs.org/"
    );
    process.exit(1);
  }
  try {
    const { values } = (0, import_util4.parseArgs)({
      options: {
    ...
    const defaultPeriodString = config.defaults.period;
    const periodString = values.period || defaultPeriodString;
    const periodHours = parseInt(periodString);
    const cutoffDate = new Date(Date.now() - periodHours * 3600 * 1e3);
    for (const feed of feeds) {
      try {
        const feedId = feed.name || new URL(feed.url).hostname;
        const items = await fetchFeed(feed.url, feedId, cutoffDate);
