...
var ConfigSchema = external_exports.object({
  ...
  defaults: external_exports.object({
    period: external_exports.string().default("24h"),
    fields: external_exports.array(external_exports.string()).default(["title", "link", "pubDate"]),
    storagePath: external_exports.string().optional()
  }).default({})
});
async function loadConfig(path4) {
  const content = await import_promises.default.readFile(path4, "utf-8");
  ...
  for (let i = 0; i < xmlData.length; i++) {
    if (xmlData[i] === "<" && xmlData[i + 1] === "?") {
      ...
      if (xmlData[i] === "!") {
        ...
        tagName19 = tagName19.trim();
        if (tagName19[tagName19.length - 1] === "/") {
          tagName19 = tagName19.substring(0, tagName19.length - 1);
          i--;
        }
        if (!validateTagName(tagName19)) {
          let msg;
          if (tagName19.trim().length === 0) {
            msg = "Invalid space after '<'.";
          } else {
            msg = "Tag '" + tagName19 + "' is an invalid name.";
          }
          return getErrorObject("InvalidTag", msg, getLineNumberForPosition(xmlData, i));
        }
        const result = readAttributeStr(xmlData, i);
        if (result === false) {
          return getErrorObject("InvalidAttr", "Attributes for '" + tagName19 + "' have open quote.", getLineNumberForPosition(xmlData, i));
        }
        ...
        if (attrStr[attrStr.length - 1] === "/") {
          ...
          if (!result.tagClosed) {
            return getErrorObject("InvalidTag", "Closing tag '" + tagName19 + "' doesn't have proper closing.", getLineNumberForPosition(xmlData, i));
            ...
            if (tagName19 !== otg.tagName) {
              ...
              return getErrorObject(
                "InvalidTag",
                "Expected closing tag '" + otg.tagName + "' (opened in line " + openPos.line + ", col " + openPos.col + ") instead of closing tag '" + tagName19 + "'.",
...
var defaultOptions2 = {
  ...
  tagValueProcessor: function(tagName19, val) {
    return val;
  },
...
var Matcher = class {
   ...
   */
  push(tagName19, attrValues = null, namespace = null) {
    this._pathStringCache = null;
    ...
    const siblingKey = namespace ? `${namespace}:${tagName19}` : tagName19;
...
function parseTextData(val, tagName19, jPath, dontTrim, hasAttributes, isLeafNode, escapeEntities) {
  ...
  if (val !== void 0) {
    ...
    if (val.length > 0) {
      if (!escapeEntities) val = this.replaceEntitiesValue(val, tagName19, jPath);
      const jPathOrMatcher = options.jPath ? jPath.toString() : jPath;
      const newval = options.tagValueProcessor(tagName19, val, jPathOrMatcher, hasAttributes, isLeafNode);
  ...
  if (force === true || options.ignoreAttributes !== true && typeof attrStr === "string") {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (attrName.length && oldVal !== void 0) {
        ...
        val = this.replaceEntitiesValue(val, tagName19, this.readonlyMatcher);
        processedVals[i] = val;
  ...
  for (let i = 0; i < xmlLen; i++) {
    ...
    if (ch === "<") {
      ...
      if (c1 === 47) {
        ...
        let tagName19 = xmlData.substring(i + 2, closeIndex).trim();
        if (options.removeNSPrefix) {
          const colonIndex = tagName19.indexOf(":");
          if (colonIndex !== -1) {
            tagName19 = tagName19.substr(colonIndex + 1);
          }
        }
        tagName19 = transformTagName(options.transformTagName, tagName19, "", options).tagName;
        if (currentNode) {
          textData = this.saveTextToParentTag(textData, currentNode, this.readonlyMatcher);
        }
        const lastTagName = this.matcher.getCurrentTag();
        if (tagName19 && options.unpairedTagsSet.has(tagName19)) {
          throw new Error(`Unpaired tag can not be used as closing tag: </${tagName19}>`);
        }
        ...
        let tagName19 = result.tagName;
        const rawTagName = result.rawTagName;
        let tagExp = result.tagExp;
        let attrExpPresent = result.attrExpPresent;
        let closeIndex = result.closeIndex;
        ({ tagName: tagName19, tagExp } = transformTagName(options.transformTagName, tagName19, tagExp, options));
        if (options.strictReservedNames && (tagName19 === options.commentPropName || tagName19 === options.cdataPropName || tagName19 === options.textNodeName || tagName19 === options.attributesGroupName)) {
          throw new Error(`Invalid tag name: ${tagName19}`);
        }
        ...
        if (tagExp.length > 0 && tagExp.lastIndexOf("/") === tagExp.length - 1) {
          isSelfClosing = true;
          if (tagName19[tagName19.length - 1] === "/") {
            tagName19 = tagName19.substr(0, tagName19.length - 1);
            tagExp = tagName19;
          } else {
            tagExp = tagExp.substr(0, tagExp.length - 1);
          }
          attrExpPresent = tagName19 !== tagExp;
        }
        let prefixedAttrs = null;
        let rawAttrs = {};
        let namespace = void 0;
        namespace = extractNamespace(rawTagName);
        if (tagName19 !== xmlObj.tagname) {
          this.matcher.push(tagName19, {}, namespace);
        }
        if (tagName19 !== tagExp && attrExpPresent) {
          prefixedAttrs = this.buildAttributesMap(tagExp, this.matcher, tagName19);
          if (prefixedAttrs) {
        ...
        if (this.isCurrentNodeStopNode) {
          ...
          if (isSelfClosing) {
            i = result.closeIndex;
          } else if (options.unpairedTagsSet.has(tagName19)) {
            i = result.closeIndex;
          } else {
          ...
          const childNode = new XmlNode(tagName19);
          if (prefixedAttrs) {
...
function replaceEntitiesValue(val, tagName19, jPath) {
  ...
  if (entityConfig.allowedTags) {
    ...
    const allowed = Array.isArray(entityConfig.allowedTags) ? entityConfig.allowedTags.includes(tagName19) : entityConfig.allowedTags(tagName19, jPathOrMatcher);
    if (!allowed) {
      return val;
    }
  }
  if (entityConfig.tagFilter) {
    const jPathOrMatcher = this.options.jPath ? jPath.toString() : jPath;
    if (!entityConfig.tagFilter(tagName19, jPathOrMatcher)) {
      return val;
    }
  }
...
function readTagExp(xmlData, i, removeNSPrefix, closingChar = ">") {
  ...
  let tagName19 = tagExp;
  let attrExpPresent = true;
  if (separatorIndex !== -1) {
    tagName19 = tagExp.substring(0, separatorIndex);
    tagExp = tagExp.substring(separatorIndex + 1).trimStart();
  }
  const rawTagName = tagName19;
  if (removeNSPrefix) {
    const colonIndex = tagName19.indexOf(":");
    if (colonIndex !== -1) {
      tagName19 = tagName19.substr(colonIndex + 1);
      attrExpPresent = tagName19 !== result.data.substr(colonIndex + 1);
    }
  }
  return {
    tagName: tagName19,
...
function readStopNodeData(xmlData, tagName19, i) {
  const startIndex = i;
  let openTagCount = 1;
  const xmllen = xmlData.length;
  for (; i < xmllen; i++) {
    if (xmlData[i] === "<") {
      const c1 = xmlData.charCodeAt(i + 1);
      if (c1 === 47) {
        const closeIndex = findClosingChar(xmlData, ">", i, `${tagName19} is not closed`);
        let closeTagName = xmlData.substring(i + 2, closeIndex).trim();
        if (closeTagName === tagName19) {
          openTagCount--;
          if (openTagCount === 0) {
            return {
        ...
        if (tagData) {
          ...
          if (openTagName === tagName19 && tagData.tagExp[tagData.tagExp.length - 1] !== "/") {
            openTagCount++;
          }
...
function transformTagName(fn, tagName19, tagExp, options) {
  if (fn) {
    const newTagName = fn(tagName19);
    if (tagExp === tagName19) {
      tagExp = newTagName;
    }
    tagName19 = newTagName;
  }
  tagName19 = sanitizeName(tagName19, options);
  return { tagName: tagName19, tagExp };
}
...
async function fetchFeed(url, name) {
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch feed from ${url}: ${response.status} ${response.statusText}`);
  }
  ...
  if (text2.trim().startsWith("{")) {
    try {
      ...
      return items.map((item) => normalizeItem(item, name));
    } catch (e) {
      throw new Error(
        `Failed to parse JSON feed from ${url}: ${e instanceof Error ? e.message : String(e)}`,
        { cause: e }
      );
    }
  } else {
    const parser = new XMLParser({
      ...
      attributeNamePrefix: "",
      processEntities: {
        maxTotalExpansions: 2e4
      }
    });
    try {
      const result = parser.parse(text2);
      const channel = result.rss?.channel || result.feed;
      const items = channel?.item || channel?.entry || [];
      return (Array.isArray(items) ? items : [items]).map((item) => normalizeItem(item, name));
    } catch (e) {
      throw new Error(
        `Failed to parse XML feed from ${url}: ${e instanceof Error ? e.message : String(e)}`,
        { cause: e }
      );
    }
  }
...
function normalizeItem(item, source) {
  const extractText = (val) => {
    const text2 = typeof val === "string" ? val : val?.["#text"] ?? (val ? String(val) : "");
    return import_he.default.decode(text2);
  };
  const truncateDescription = (text2) => {
    const stripped = text2.replace(/<[^>]*>?/gm, "");
    const lines = stripped.split(/\r?\n/).map((l) => l.trim()).filter((l) => {
      if (l.length < 2) return false;
      const lower = l.toLowerCase();
      if (lower === "view csaf") return false;
      if (lower === "expand all +") return false;
      if (lower === "summary") return false;
      if (lower === "background") return false;
      return true;
    });
  ...
  const extractLink = (link2) => {
    if (typeof link2 === "string") return link2;
    if (Array.isArray(link2)) {
      const alternate = link2.find((l) => l.rel === "alternate" || !l.rel);
      return extractLink(alternate || link2[0]);
    }
    if (link2 && typeof link2 === "object") return link2.href || link2["#text"] || "";
    return "";
  };
  const title = extractText(item.title || item.headline);
  const link = extractLink(item.link);
  const pubDateRaw = item.pubDate || item.published || item.updated || Date.now();
  let pubDate;
  ...
  const id = import_crypto.default.createHash("md5").update(`${title}|${link}`).digest("hex");
  return {
    id,
    title,
    link,
    pubDate,
    source,
    description: truncateDescription(extractText(item.description || item.summary))
  };
}
...
function sanitizeFeedId(feedId) {
  return feedId.toLowerCase().replace(/[^a-z0-9]/g, "_");
}
async function saveSnapshot(feedId, items, dataDir = ".security-rss-feed/data") {
  const sanitizedId = sanitizeFeedId(feedId);
  await import_promises2.default.mkdir(dataDir, { recursive: true });
  const content = items.map((item) => JSON.stringify(item)).join("\n");
  const filePath = import_path.default.join(dataDir, `${sanitizedId}.jsonl`);
  await import_promises2.default.writeFile(filePath, content);
}
...
function createDOMPurify() {
  let window4 = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : getGlobal();
  const DOMPurify2 = (root) => createDOMPurify(root);
  DOMPurify2.version = "3.4.3";
  DOMPurify2.removed = [];
  if (!window4 || !window4.document || window4.document.nodeType !== NODE_TYPE.document || !window4.Element) {
    DOMPurify2.isSupported = false;
    return DOMPurify2;
  }
  let document = window4.document;
  const originalDocument = document;
  const currentScript = originalDocument.currentScript;
  const DocumentFragment3 = window4.DocumentFragment, HTMLTemplateElement2 = window4.HTMLTemplateElement, Node4 = window4.Node, Element4 = window4.Element, NodeFilter2 = window4.NodeFilter, _window$NamedNodeMap = window4.NamedNodeMap, NamedNodeMap2 = _window$NamedNodeMap === void 0 ? window4.NamedNodeMap || window4.MozNamedAttrMap : _window$NamedNodeMap, HTMLFormElement2 = window4.HTMLFormElement, DOMParser2 = window4.DOMParser, trustedTypes = window4.trustedTypes;
  ...
  let emptyHTML = "";
  const _document = document, implementation = _document.implementation, createNodeIterator = _document.createNodeIterator, createDocumentFragment = _document.createDocumentFragment, getElementsByTagName2 = _document.getElementsByTagName;
  ...
  const _checkValidNamespace = function _checkValidNamespace2(element) {
    ...
    const tagName19 = stringToLowerCase(element.tagName);
    ...
    if (element.namespaceURI === SVG_NAMESPACE2) {
      if (parent.namespaceURI === HTML_NAMESPACE) {
        return tagName19 === "svg";
      }
      if (parent.namespaceURI === MATHML_NAMESPACE) {
        return tagName19 === "svg" && (parentTagName === "annotation-xml" || MATHML_TEXT_INTEGRATION_POINTS[parentTagName]);
      }
      return Boolean(ALL_SVG_TAGS[tagName19]);
    }
    if (element.namespaceURI === MATHML_NAMESPACE) {
      if (parent.namespaceURI === HTML_NAMESPACE) {
        return tagName19 === "math";
      }
      if (parent.namespaceURI === SVG_NAMESPACE2) {
        return tagName19 === "math" && HTML_INTEGRATION_POINTS[parentTagName];
      }
      return Boolean(ALL_MATHML_TAGS[tagName19]);
    }
    if (element.namespaceURI === HTML_NAMESPACE) {
      ...
      return !ALL_MATHML_TAGS[tagName19] && (COMMON_SVG_AND_HTML_ELEMENTS[tagName19] || !ALL_SVG_TAGS[tagName19]);
    }
  ...
  const _isClobbered = function _isClobbered2(element) {
    return element instanceof HTMLFormElement2 && (typeof element.nodeName !== "string" || typeof element.textContent !== "string" || typeof element.removeChild !== "function" || !(element.attributes instanceof NamedNodeMap2) || typeof element.removeAttribute !== "function" || typeof element.setAttribute !== "function" || typeof element.namespaceURI !== "string" || typeof element.insertBefore !== "function" || typeof element.hasChildNodes !== "function");
  };
  ...
  const _sanitizeElements = function _sanitizeElements2(currentNode) {
    ...
    const tagName19 = transformCaseFunc(currentNode.nodeName);
    ...
    if (SAFE_FOR_XML && currentNode.namespaceURI === HTML_NAMESPACE && tagName19 === "style" && _isNode(currentNode.firstElementChild)) {
      _forceRemove(currentNode);
      return true;
    }
    ...
    if (FORBID_TAGS[tagName19] || !(EXTRA_ELEMENT_HANDLING.tagCheck instanceof Function && EXTRA_ELEMENT_HANDLING.tagCheck(tagName19)) && !ALLOWED_TAGS[tagName19]) {
      if (!FORBID_TAGS[tagName19] && _isBasicCustomElement(tagName19)) {
        if (CUSTOM_ELEMENT_HANDLING.tagNameCheck instanceof RegExp && regExpTest(CUSTOM_ELEMENT_HANDLING.tagNameCheck, tagName19)) {
          return false;
        }
        if (CUSTOM_ELEMENT_HANDLING.tagNameCheck instanceof Function && CUSTOM_ELEMENT_HANDLING.tagNameCheck(tagName19)) {
          return false;
        }
      }
      if (KEEP_CONTENT && !FORBID_CONTENTS[tagName19]) {
    ...
    if ((tagName19 === "noscript" || tagName19 === "noembed" || tagName19 === "noframes") && regExpTest(/<\/no(script|embed|frames)/i, currentNode.innerHTML)) {
      _forceRemove(currentNode);
      return true;
    }
  ...
  const _isBasicCustomElement = function _isBasicCustomElement2(tagName19) {
    return !RESERVED_CUSTOM_ELEMENT_NAMES[stringToLowerCase(tagName19)] && regExpTest(CUSTOM_ELEMENT$1, tagName19);
  };
  ...
  const _sanitizeAttachedShadowRoots2 = function _sanitizeAttachedShadowRoots(root) {
    if (root.nodeType === NODE_TYPE.element && root.shadowRoot instanceof DocumentFragment3) {
  ...
  DOMPurify2.sanitize = function(dirty) {
    ...
    if (IN_PLACE) {
      ...
      if (typeof nn === "string") {
        const tagName19 = transformCaseFunc(nn);
        if (!ALLOWED_TAGS[tagName19] || FORBID_TAGS[tagName19]) {
  ...
  upgrade(element) {
    ...
    if (registry.has(ce)) {
      ...
      if (check(element)) {
        ...
        const values = entries2(element);
        for (const [key2] of values)
          delete element[key2];
        setPrototypeOf2(element, Class.prototype);
...
var filters = {
  ...
  // T_O_D_O determine the actual root element
...
function convertOptionFormats(options) {
  var _a3, _b, _c, _d;
  const opts = options !== null && options !== void 0 ? options : defaultOptions3;
...
setPrototypeOf2(DOMStringMap.prototype, null);
...
var create3 = (ownerDocument, element, localName) => {
...
var Element2 = class extends ParentNode {
  ...
  set textContent(text2) {
    ...
    if (text2 != null && text2 !== "")
      this.appendChild(new Text3(this.ownerDocument, text2));
  }
  get innerHTML() {
    return getInnerHtml(this);
  }
  ...
  cloneNode(deep = false) {
    ...
    const clone2 = create3(ownerDocument, this, localName);
    ...
    while (next !== prev && (deep || next.nodeType === ATTRIBUTE_NODE)) {
      switch (next.nodeType) {
        case NODE_END:
          ...
          break;
        case ELEMENT_NODE: {
          const node = create3(ownerDocument, next, next.localName);
          addNext(node);
          parentNode = node;
          break;
        }
...
var Range = class _Range {
  ...
  /* T_O_D_O: this is more complicated than it looks
  ...
  // T_O_D_O: SVG elements should then create contextual fragments
...
var Document2 = class extends NonElementParentNode {
  ...
  createEvent(name) {
    const event = create2(name === "Event" ? new GlobalEvent("") : new CustomEvent(""));
...
var parseHTML = (html2, globals = null) => new DOMParser().parseFromString(
  html2,
  "text/html",
  globals
).defaultView;
...
setPrototypeOf2(Document4, Document2).prototype = Document2.prototype;

// src/template.ts
var { window: window3 } = parseHTML("<!DOCTYPE html><html><body></body></html>");
var purifyFactory = purify.default || purify;
var DOMPurify = purifyFactory(window3);
...
function renderReport(items, periodHours) {
  const dateStr = (/* @__PURE__ */ new Date()).toLocaleString();
  const periodStr = formatPeriod(periodHours);
  const itemsHtml = items.map(
    ...
    <article class="feed-item">
      ...
      <div class="meta">
        ...
        <time datetime="${escapeHtml(item.pubDate)}">${new Date(item.pubDate).toLocaleString()}</time>
      </div>
      ${item.description ? `<div class="description">${DOMPurify.sanitize(item.description)}</div>` : ""}
    </article>
  `
  ).join("\n");
  return `
...
<head>
    ...
    <title>Security RSS Feed Report - ${dateStr}</title>
    <style>
        body {
...
<body>
    <header>
        <h1>Security RSS Feed Report</h1>
        <div class="report-meta">
            Covering the last ${periodStr} \u2014 Generated on ${dateStr}
        </div>
    </header>
    <main>
        ${items.length > 0 ? itemsHtml : "<p>No new items found in this period.</p>"}
    </main>
    <footer>
        Generated by Security RSS Feed Agent
    </footer>
</body>
</html>
  `.trim();
...
// src/reporter.ts
async function generateReport(periodHours = 24, dataDir = ".security-rss-feed/data", reportDir = ".security-rss-feed/report", feedIds) {
  const files = await import_promises3.default.readdir(dataDir).catch(() => []);
  const allowedFiles = feedIds?.map((id) => `${sanitizeFeedId(id)}.jsonl`);
  const filteredFiles = allowedFiles ? files.filter((f) => allowedFiles.includes(f)) : files.filter((f) => f.endsWith(".jsonl"));
  const allItems = [];
  for (const file of filteredFiles) {
    const content = await import_promises3.default.readFile(import_path2.default.join(dataDir, file), "utf-8");
    const lines = content.split("\n").filter((l) => l.trim());
    allItems.push(...lines.map((l) => JSON.parse(l)));
  }
  const cutoff = new Date(Date.now() - periodHours * 60 * 60 * 1e3);
  const itemMap = /* @__PURE__ */ new Map();
  for (const item of allItems) {
    if (new Date(item.pubDate) >= cutoff) {
  ...
  const html2 = renderReport(uniqueItems, periodHours);
  await import_promises3.default.mkdir(reportDir, { recursive: true });
  const reportPath = import_path2.default.join(reportDir, `report-${Date.now()}.html`);
  await import_promises3.default.writeFile(reportPath, html2);
  return reportPath;
}
...
async function main() {
  ...
  if (majorVersion < 24) {
    console.error(`Error: This application requires Node.js v24 or higher (detected ${process.version}).`);
    console.error("Please install the necessary Node.js runtime from https://nodejs.org/");
    process.exit(1);
  }
  try {
    const { values } = (0, import_util7.parseArgs)({
      options: {
        config: { type: "string", short: "c", default: "assets/settings.json" },
    ...
    const storageRoot = config.defaults.storagePath || import_path3.default.join(import_os.default.tmpdir(), "security-rss-feed");
    const dataDir = import_path3.default.join(storageRoot, "data");
    const reportDir = import_path3.default.join(storageRoot, "report");
    if (values.url) {
      const urls = values.url;
      const names = values.name || [];
      for (let i = 0; i < urls.length; i++) {
        feeds.push({ url: urls[i], name: names[i] || `Ad-hoc Feed ${i + 1}` });
      }
    }
    console.log(`Security RSS Feed: Scanning ${feeds.length} feeds...`);
    console.log(`Storage root: ${storageRoot}`);
    const processedFeedIds = [];
    for (const feed of feeds) {
      try {
        const feedId = feed.name || new URL(feed.url).hostname;
        const items = await fetchFeed(feed.url, feedId);
        await saveSnapshot(feedId, items, dataDir);
        processedFeedIds.push(feedId);
        console.log(`  \u2713 ${feedId}: ${items.length} items snapshot saved.`);
      } catch (e) {
        console.error(`  \u2717 Failed to fetch ${feed.url}: ${e.message}`);
      }
    }
    const defaultPeriod = parseInt(config.defaults.period);
    const period = values.period ? parseInt(values.period) : defaultPeriod;
    const reportPath = await generateReport(period, dataDir, reportDir, processedFeedIds);
    console.log(`
